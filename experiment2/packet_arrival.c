...
packet_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
    ...
    Packet new_packet;
    unsigned packet_sizes[] = {500, 1000, 1500, 2000, 2500};
    new_packet.arrive_time = simulation_run_get_time(simulation_run);
    int selection = (int) floor((uniform_generator() * 5));
    new_packet.size = packet_sizes[selection];
    new_packet.service_time = (double) 1/data->service_rate * new_packet.size;
    data->arrival_count++;
